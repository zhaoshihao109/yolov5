...
model_weight_path = "D:\PYTHON_work_space\SSH\yolov5_brief\\runs\\train\exp\weights\\best.pt"
...
if __name__ == '__main__':
    ...
    plt.figure()
    plt.subplot(2, 3, 1)
    plt.bar(range(len(name_list)), num_lists[0], color='rgb', tick_label=name_list)
    plt.subplot(2, 3, 2)
    plt.bar(range(len(name_list)), num_lists[1], color='rgb', tick_label=name_list)
    plt.subplot(2, 3, 3)
    plt.bar(range(len(name_list)), num_lists[2], color='rgb', tick_label=name_list)
    plt.subplot(2, 3, 4)
    plt.bar(range(len(name_list)), num_lists[3], color='rgb', tick_label=name_list)
    plt.subplot(2, 3, 5)
    plt.bar(range(len(name_list)), num_lists[4], color='rgb', tick_label=name_list)
    plt.savefig('teaBar.jpg')
    # plt.show()
